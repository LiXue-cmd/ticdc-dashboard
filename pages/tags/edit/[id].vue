<template>
  <div class="w-full max-w-2xl mx-auto p-6 bg-white rounded-lg shadow-sm">
    <h2 class="text-xl font-bold mb-4">编辑标签 - {{ form.name }}</h2>
...
onMounted(async () => {
  ...
  if (!tagId) return router.push("/tags"); // 无ID时返回列表页

  try {
    isLoading.value = true;
    const { data, error } = await useFetch(`/api/tags/${tagId}`);
...
const handleSubmit = async () => {
  ...
  if (!tagId) return toast({ title: "错误", description: "标签ID缺失" });
